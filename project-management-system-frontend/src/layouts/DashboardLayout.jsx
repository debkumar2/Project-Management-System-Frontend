...
export default function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#F4F6F8] dark:bg-[#111827] text-[#111827] dark:text-[#F3F4F6] font-sans transition-colors duration-300">
      ...
      >
        {/* Workspace Switcher */}
        <div className="h-16 px-4 flex items-center shrink-0 border-b border-[#E5E7EB] dark:border-[#374151]">
          <button className="flex items-center gap-3 w-full p-2 rounded-xl transition-all hover:bg-white dark:hover:bg-[#374151]/50 group overflow-hidden shadow-sm hover:shadow border border-transparent hover:border-[#E5E7EB] dark:hover:border-[#4B5563]">
            ...
            {isSidebarOpen && (
              <div className="flex-1 min-w-0 flex items-center justify-between opacity-100 whitespace-nowrap transition-opacity duration-300">
                <div className="flex flex-col items-start">
                  <span className="font-semibold text-[14px] truncate text-[#111827] dark:text-white leading-snug">FlowForge</span>
                  <span className="text-[11px] font-medium text-[#6B7280] dark:text-[#9CA3AF] leading-none">Enterprise</span>
                </div>
                <ChevronDown size={14} className="text-[#9CA3AF] group-hover:text-[#6B7280] dark:group-hover:text-white transition-colors" />
              </div>
            )}
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative z-10">
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar relative w-full flex justify-center bg-[#F4F6F8] dark:bg-[#111827] transition-colors duration-300">
          <div className="w-full max-w-[1440px] px-8 py-8">
            <Outlet />
          </div>
        </main>
      </div>
